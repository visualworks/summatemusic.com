window.addEventListener("load", () => {
    ...
    const now = new Date();
    const isStreamingTime = (now.getUTCDate() === 10 || now.getUTCDate() === 12) && now.getUTCHours() >= 20 && now.getUTCHours() <= 23;
    const isDayInterval = (now.getUTCDate() > 8 && now.getUTCDate() < 10) || (now.getUTCDate() === 10 && now.getUTCHours() >= 23);
    if (isStreamingTime) {
        ...
        if (isDayInterval) {
            ...
            setInterval(() => {
                const now = new Date();
                const startUTCTime = Date.UTC("2019", "11", "10", "20", "00", "00");
